...
const logicSeePetLogLikes: Resolver = async(_,{id,cursorId},{client,loggedInUser}) => {
...
const seePetLogLikesFn: Resolver = async(_,props,ctx) => {
  return paginationErrorCheckNeedLogicAndQueryName(
    await logicSeePetLogLikes(_,props,ctx,null),
    "seePetLogLikes"
  );
};
...
const resolver: Resolvers = {
  Query: {
    seePetLogLikes: seePetLogLikesFn,
  },
};
