...
const logicSeePostLikes: Resolver = async(_,{id,cursorId},{client,loggedInUser}) => {
...
const seePostLikesFn:Resolver = async(_,props,ctx) => {
  return paginationErrorCheckNeedLogicAndQueryName(
    await logicSeePostLikes(_,props,ctx,null),
    "seePostLikes"
  );
};
